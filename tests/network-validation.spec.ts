...
test.describe("Network & API Validation", () => {
  ...
  test("should verify Mixpanel API endpoint and payload structure", async ({
    page,
  }) => {
    ...
    try {
      ...
      console.log(
        "No events captured, but continuing with network validation...",
      );
    }
    ...
    const mixpanelApiCalls = allRequests.filter(
      (request) =>
    ...
    for (const request of mixpanelApiCalls) {
      ...
      if (url.includes("/track/")) {
        ...
        try {
          ...
          if (dataParam) {
            const decodedPayload = Buffer.from(dataParam, "base64").toString(
              "utf-8",
            );
  ...
  test("should validate event properties and data quality", async ({
    page,
  }) => {
    const mainUrl =
      process.env.STAGING_BASE_URL || "https://gatotkaca.tiket.com";
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (event.properties) {
        ...
        const hasUserAgent =
          event.properties.user_agent || event.properties.$user_agent;
        console.log(`- Has user agent: ${!!hasUserAgent}`);

        // Check for screen resolution
        const hasScreen =
          event.properties.screen_width || event.properties.$screen_width;
  ...
  test("should monitor network performance and response times", async ({
    page,
  }) => {
  ...
  test("should verify cross-environment consistency", async ({ page }) => {
    ...
    for (const event of events) {
      if (event.properties) {
        ...
        const urlProperty =
          event.properties.url || event.properties.$current_url;
        if (urlProperty) {
